...
import com.skillbrain.store.controller.OrderController;
import com.skillbrain.store.controller.ProductController;
import com.skillbrain.store.dto.OrderProductDto;
...
public class SausageApplicationIntegrationTest {
    ...
    public void givenGetProductsApiCall_whenProductListRetrieved_thenSizeMatchAndListContainsProductNames() {
        ...
        assertThat(products, hasItem(hasProperty("name", is("Cremoasa"))));
        assertThat(products, hasItem(hasProperty("name", is("Speciala"))));
        assertThat(products, hasItem(hasProperty("name", is("Cu lapte"))));
        assertThat(products, hasItem(hasProperty("name", is("Nurnberg"))));
        assertThat(products, hasItem(hasProperty("name", is("Munchen"))));
        assertThat(products, hasItem(hasProperty("name", is("Ruseasca"))));
    }
    ...
    private OrderController.OrderForm prepareOrderForm() {
        ...
        productDto.setProduct(new Product(1L, "Ruseasca", 300.00, "http://placehold.it/200x100"));
